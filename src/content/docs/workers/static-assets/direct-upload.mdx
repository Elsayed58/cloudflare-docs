...
The [file upload API](/api/operations/worker-assets-upload) requires files be uploaded using multipart/form data. The contents of each file must be base64 encoded, and the `base64` query param must be set to `true`.
